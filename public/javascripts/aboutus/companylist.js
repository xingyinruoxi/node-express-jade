...
window.companyList = (function () {
    ...
    function getListHtml(datalist) {
        ...
        if (datalist) {
            ...
            for (var i = 0; i < max; i++) {
                html += '<li class="list">'
                    + ' <div class="list_body">'
                    + ' <h4><a href="detail'+dataList[i].id+'.html">'+dataList[i].topic+'</a></h4>'
                    + '  <p>'+dataList[i].brief+'</p>'
                    + '  <time class="color-gray">'+date.formatDateTime(dataList[i].reportDate)+'</time><span class="color-gray">文章来源： '+dataList[i].source+'</span>'
                    + ' </div>'
                    + ' <div class="list_link">'
                    + ' <a href="detail'+dataList[i].id+'.html"><img src="'+(dataList[i].coverImgAlt.indexOf('http')>-1?dataList[i].coverImgAlt:imageHost+dataList[i].coverImgAlt)+'" width="194" height="132"></a>'
    ...
    //获取公司动态表数据
    function getCompanyList(pageNo, callback) {
        var paras = {
