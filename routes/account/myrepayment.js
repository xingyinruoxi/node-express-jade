...
router.post('/get_tradelist.json', function (req, res) {
    ...
    co(function* () {
        try {
            data = yield APIClient.post(APIPath.ALICLOUD_ACCOUNT_MYTRADETENDER, {}, {}, body,token);
...
router.post('/get_repaylist.json', function (req, res) {
    ...
    co(function* () {
        try {
            data = yield APIClient.post(APIPath.ALICLOUD_ACCOUNT_MYTRADERREPAYMENT, {}, {},body,token);
            console.log('data', data);
        } catch (e) {
            data = {};
            console.log('验证手机号是否存在error', data, e.message);
